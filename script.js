document.addEventListener("DOMContentLoaded", () => {
    ...
    let skippedSteps = {}; // Track skipped journey steps per specialty: { specialty: Set<index> }
    let currentSuggestedStepIndex = null;
    ...
    if (specialtySelector) {
        specialtySelector.addEventListener("change", () => {
            currentSuggestedStepIndex = null;
            updateLayout();
        });
    }
    ...
    document.getElementById('copilot-accept-btn').addEventListener('click', () => {
        if (!pendingRecommendation) return;
        const rec = pendingRecommendation;

        // Create real node from the recommendation, using duration from journey data
    ...
    // Skip Suggestion button handler
    document.getElementById('copilot-skip-btn').addEventListener('click', () => {
        const specialty = document.getElementById('specialty-selector').value;
        if (!skippedSteps[specialty]) skippedSteps[specialty] = new Set();
        if (currentSuggestedStepIndex !== null) {
            skippedSteps[specialty].add(currentSuggestedStepIndex);
        }
        pendingRecommendation = null;
        updateLayout();
    });
    ...
    gridContainer.addEventListener("drop", (e) => {
        ...
        if (currentMode === "quarterly" && !existingId) {
            // Snap NEW nodes to the first day of the month and span the entire month
        ...
        if (existingId) {
            ...
            // Create new node
            const currentSpecialty = document.getElementById("specialty-selector")?.value || "cardiologists";
            const newNode = {
                id: "node-" + Math.random().toString(36).substring(2, 9),
    ...
    function updateLayout() {
        ...
        const isCopilotEnabled = document.getElementById('toggle-recommendations')?.checked ?? true;
        ...
        if (isCopilotEnabled && recommendationData && activeChannels.length > 0) {
            ...
            if (journey) {
                // Only consider nodes placed under the CURRENT specialty for matching and continuity
                const realNodes = nodesData.filter(n => !n.isGhost && n.specialty === specialty).sort((a, b) => a.timestamp - b.timestamp);
                const specialtySkips = skippedSteps[specialty] || new Set();
                let lastMatchedNode = null;
                let nextStepIndex = 0;
               
                for (let i = 0; i < journey.length; i++) {
                    if (specialtySkips.has(i)) {
                        nextStepIndex = i + 1;
                        continue;
                    }
                    const step = journey[i];
                    const match = realNodes.find(n => n.type === step.tacticType && (!lastMatchedNode || n.timestamp >= lastMatchedNode.timestamp));
                    if (match) {
                ...
                // Skip over explicitly skipped steps
                while (nextStepIndex < journey.length && specialtySkips.has(nextStepIndex)) {
                    nextStepIndex++;
                }
               
                // Trace forward from lastMatchedNode through connections
                // to find the actual terminal node (handles manually placed tactics downstream)
                let anchorNode = lastMatchedNode;
                if (anchorNode) {
                    let visited = new Set();
                    let current = anchorNode;
                    while (current) {
                        visited.add(current.id);
                        const outgoing = connections.filter(c => c.sourceId === current.id && !c.isGhost);
                        if (outgoing.length === 0) break;
                        let latestTarget = null;
                        let latestTs = -Infinity;
                        for (const conn of outgoing) {
                            const target = nodesData.find(n => n.id === conn.targetId && !n.isGhost);
                            if (target && !visited.has(target.id) && target.timestamp >= latestTs) {
                                latestTs = target.timestamp;
                                latestTarget = target;
                            }
                        }
                ...
                // Segment switching: if no journey match but tactics exist,
                // connect to the last tactic by timestamp for continuity
                if (!anchorNode && realNodes.length > 0) {
                    anchorNode = realNodes[realNodes.length - 1];
                }
               
                currentSuggestedStepIndex = nextStepIndex;
               
                // Debug logging
                console.log('[JSW Copilot]', {
                    specialty,
                    nextStepIndex,
                    lastMatchedNode: lastMatchedNode ? { id: lastMatchedNode.id, name: lastMatchedNode.name, type: lastMatchedNode.type } : null,
                    anchorNode: anchorNode ? { id: anchorNode.id, name: anchorNode.name, type: anchorNode.type } : null,
                    realNodesCount: realNodes.length,
                    skippedSteps: Array.from(specialtySkips)
                });
               
                if (nextStepIndex < journey.length) {
                    const nextStep = journey[nextStepIndex];
                    const tacticDisplayName = tacticNames[nextStep.tacticType] || nextStep.tacticType;
                   
                    // Update Copilot UI
                    document.getElementById('copilot-tactic').innerText = tacticDisplayName;
                    document.getElementById('copilot-step-counter').innerText = `Step ${nextStepIndex + 1} of ${journey.length}`;
                    if (anchorNode) {
                        const anchorName = anchorNode.name || 'previous tactic';
                        document.getElementById('copilot-timing').innerText = `${nextStep.delayDays} days after "${anchorName}". Connect with '${nextStep.connectionType}' line.`;
                    } else {
                        document.getElementById('copilot-timing').innerText = `Place this tactic at the start of your timeline.`;
                    }
                    document.getElementById('copilot-rationale').innerText = nextStep.rationale;
                    copilotWidget.style.display = 'block';
                   
                    // Calculate ghost position
                    const dayMs = 24 * 60 * 60 * 1000;
                    const duration = nextStep.durationDays || (currentMode === 'quarterly' ? 30 : 1);
                    const startMs = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate()).getTime();
                    let ghostTimestamp;
                    if (anchorNode) {
                        const anchorEnd = anchorNode.timestamp + (anchorNode.durationDays || 1) * dayMs;
                        ghostTimestamp = anchorEnd + (nextStep.delayDays * dayMs);
                    } else {
                        ghostTimestamp = startMs;
                    }
                   
                    // Find suitable lane
                    let targetLaneId = activeChannels[0].id;
                    if (anchorNode && activeChannels.find(c => c.id === anchorNode.laneId)) {
                        targetLaneId = anchorNode.laneId;
                    }
                    ...
                    pendingRecommendation = {
                        ...
                        laneId: targetLaneId,
                        connectionType: nextStep.connectionType || (anchorNode ? 'next' : null),
                        sourceId: anchorNode ? anchorNode.id : null,
                    ...
                    nodesData.push({
                        ...
                        laneId: targetLaneId,
                        type: nextStep.tacticType,
                        name: 'Suggested: ' + tacticDisplayName,
                        timestamp: ghostTimestamp,
                        durationDays: duration,
                        isGhost: true,
                        isStandalone: false
                    ...
                    // Determine connection type: use the journey step's type,
                    // but default to 'next' when connecting to an anchor (e.g. segment switching)
                    const effectiveConnType = nextStep.connectionType || (anchorNode ? 'next' : null);
                   
                    if (anchorNode) {
                        connections.push({
                            sourceId: anchorNode.id,
                            targetId: 'ghost-node',
                            type: effectiveConnType,
                            isGhost: true
                        });
                    }
                } else {
